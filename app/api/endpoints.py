...
def recharge_account(account_id: str, amount: int, db: Session = Depends(get_db)):
...
def ui_recharge_account(account_id: str, amount: int = Form(...), db: Session = Depends(get_db)):
    db_account = db.query(AccountModel).filter(AccountModel.account_id == account_id).first()
    if db_account:
        db_account.credits += amount
        db.commit()
